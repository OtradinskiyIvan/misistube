...
class SearchVideoUseCase:
    ...
    async def execute(self, query: SearchQuery) -> SearchResponse:
        ...
        items, total = await self.search_port.search(
            query=query.q, tags=query.tags, offset=query.offset, limit=query.limit
        )

        for item in items:
            if item.thumbnail_key:
                item.thumbnail_url = await self.storage.generate_thumbnail_url(item.thumbnail_key)

        await self._enrich_usernames(items)

        response = SearchResponse(items=items, total=total, offset=query.offset, limit=query.limit)
    ...
    async def _enrich_usernames(self, items: list) -> None:
        ...
        logger.info("Fetching usernames for %d users from user service", len(usernames_to_fetch))

        fetched_usernames = await self.user_service_client.get_usernames_batch(usernames_to_fetch)
